...
void beriPakan(int gram) {
  int maksimumPerPemberian = 200;  // Maksimum pakan dalam satu kali pemberian (gram)
  int beratTerjatuh = 0;
...
void kosongkanTempatPakan() {
  ...
  while (!pembersihanSelesai) {
    ...
    if (beratSaatIni > 5) {
      ...
      // Motor tetap bekerja selama 20 detik
...
void kosongkanTempatPakan() {
  ...
  bool pembersihanSelesai = false;
  bool beratKurangDari8 = false;
  unsigned long motorStartTime = 0; // Waktu mulai stepper bergerak
  ...
  while (!pembersihanSelesai) {
    ...
    if (beratSaatIni > 5) {
      ...
      if (!beratKurangDari8) {
        beratKurangDari8 = true; // Tandai kapan berat turun di bawah 8
        motorStartTime = millis(); // Catat waktu mulai stepper bekerja selama 20 detik
      }

      // Motor tetap berputar selama 20 detik
      if (millis() - motorStartTime <= 20000) {
        myStepper.step(stepsPerRevolution / 10);
      } else {
        pembersihanSelesai = true; // Hentikan setelah 20 detik
      }
    }
    
    // Timeout tambahan jika berat tidak berubah signifikan
    if (millis() - startTime >= 25000) { // Maksimal 25 detik untuk keselamatan
      pembersihanSelesai = true;
    }
  }
