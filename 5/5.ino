...
int startDay = 0, startMonth = 0, startYear = 0, jumlahIkan = 0; // Variabel kosong pada awalnya
...
void setup() {
  ...
  preferences.begin("pakanLele", false);

  // Baca nilai dari memori jika tersedia
  startDay = preferences.getInt("startDay", 0);
  startMonth = preferences.getInt("startMonth", 0);
  startYear = preferences.getInt("startYear", 0);
  jumlahIkan = preferences.getInt("jumlahIkan", 0);
  ...
  Serial.print("jumlahIkan: "); Serial.println(jumlahIkan);
...
// Fungsi untuk mengupdate data dari Blynk
BLYNK_WRITE(V1) {
  // Menyimpan nilai yang diterima dari widget Blynk di aplikasi
  if (param.asInt() == 1) {  // Ketika tombol start ditekan
    DateTime now = rtc.now();
    startDay = now.day();
    ...
    preferences.putInt("startDay", startDay);
    preferences.putInt("startMonth", startMonth);
    preferences.putInt("startYear", startYear);

    Serial.print("Start alat dimulai pada: ");
    Serial.print(startDay); Serial.print("/");
    Serial.print(startMonth); Serial.print("/");
    Serial.println(startYear);
  }
}

BLYNK_WRITE(V4) {
  // Menyimpan nilai jumlah ikan yang diterima dari Blynk
  jumlahIkan = param.asInt();
  preferences.putInt("jumlahIkan", jumlahIkan);  // Simpan ke Preferences
  Serial.print("Jumlah ikan diperbarui: ");
...
int hitungPakan(int jumlahIkan) {
  ...
  return jumlahIkan * beratIkan;  // Menghitung total berat pakan
}
